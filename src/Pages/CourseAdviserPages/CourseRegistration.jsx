...
// Department courses data structure
...
const CourseRegistrationsView = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            if (!selectedLevel || !userDepartment) {
                setRegistrations([]);
                return;
            }
    ...
    }, [selectedLevel, userDepartment, semesterFilter]);
    ...
    const handleApproveCourse = async (registrationId, courseCode, studentId) => {
        try {
            ...
            await updateDoc(registrationDoc, {
                status: 'approved', // Updates the top-level status
    ...
    const handleRejectCourse = async (registrationId, courseCode, studentId) => {
        try {
            ...
            const registrationDoc = doc(db, 'examRegistrations', registrationId);
            await updateDoc(registrationDoc, {
                status: 'rejected', // Updates the top-level status
                rejectedAt: serverTimestamp(),
                rejectedBy: {
                    uid: auth.currentUser.uid,
                    name: auth.currentUser.displayName || 'Course Adviser'
                },
                rejectionReason: "Please contact your course adviser for details" // Optional
            });
    ...
    const groupByCourse = () => {
        ...
        registrations.forEach(reg => {
            // Skip if no courses or invalid format
            if (!reg.courses) {
            ...
            // Normalize courses to array format
            let coursesList = [];
    ...
    const getDepartmentCourses = () => {
        if (!userDepartment || !departmentCourses[userDepartment]) return [];
        return departmentCourses[userDepartment];
    };
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
                ...
                <main className="flex-1 overflow-y-auto p-6">
                    <div className="max-w-7xl mx-auto">
                        ...
                        {loading ? (
                        ...
                        ) : selectedLevel && userDepartment ? (
                            <div className="bg-white rounded-lg shadow overflow-hidden">
                                <div className="px-6 py-4 border-b border-gray-200 bg-gray-50 flex justify-between items-center">
                                    <div>
                                        <h2 className="font-semibold text-lg text-gray-800">
                                            {userDepartment} - {selectedLevel}
                                        </h2>
                                ...
                                {getDepartmentCourses().length > 0 ? (
                                    ...
                                    <div className="p-8 text-center text-gray-500">
                                        No courses found for your department ({userDepartment})
                                    </div>
                                )}
                            </div>
                        ) : (
                            <div className="bg-gray-50 rounded-lg p-8 text-center">
                                ...
                                <h3 className="mt-2 text-sm font-medium text-gray-900">
                                    {!selectedLevel ? 'Select an academic level' : 'Loading department...'}
                                </h3>
                                <p className="mt-1 text-sm text-gray-500">
                                    Please select a level to view course registrations
                                </p>
                            </div>
                        )}
                    </div>
